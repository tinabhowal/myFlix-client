...
export const MovieCard = ({ movie, user, hasFavorite, toggleFavorite }) => {
  let foundFavoritedMovies = user.FavouriteMovies;
  const handleFavoriteClick = (e) => {
    e.preventDefault();
    toggleFavorite(movie);
  };

  const favoriteButtonLabel = hasFavorite
  ...
  return (
    <Card className="h-100">
      ...
      <Card.Body>
        ...
        <Card.Text>
          <span>Director: </span> {movie.director.Name}
        </Card.Text>
        <Link to={`/movies/${encodeURIComponent(movie.id)}`}>
          <Button variant="link" onClick={handleFavoriteClick}>
            ...
            <FontAwesomeIcon
              ...
              className={
                foundFavoritedMovies.includes(movie.id) ? "text-danger" : ""
              }
            />
          </Button>
