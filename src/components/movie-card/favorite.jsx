...
//   const isFavorite = user.FavouriteMovies.find(
...
//     >
//       {isFavorite ? <FontAwesomeIcon icon={faHeart}  style={{ fontSize: '2em', padding: '0.1em' }} color="red" /> : <FontAwesomeIcon icon={faHeart} style={{ fontSize: '2em', padding: '0.1em' }}/>}
//     </Button>
  ...
  useEffect(() => {
    ...
    fetch(`https://myflix-gqp8.onrender.com/users/${user.Username}`,
      ...
      .then((data) => {
        if (!data) return;
        dispatch(setUser(data));
        setHasFavorite(data.FavouriteMovies.includes(movie.id));
      })
  }, []);
